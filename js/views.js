...
async function addCandidate(reqId) {
  openModal(`
...
function renderJDLibrary() {
  ...
  const filtered = allRoles.filter(r => {
    ...
    return (a._functionName || '').localeCompare(b._functionName || '')
        ...
        || (parseInt(a.grade, 10) || 0) - (parseInt(b.grade, 10) || 0);
  });
...
function enrichOnboarding(o) {
  ...
  return {
    ...
    reqTitle: req?.roleTitle || req?.jobTitle || req?.position_title || '—',
    reqCode: req?.id || req?.code || req?.reqCode || '—',
    reqId: req?.id || null,
...
async function openEditChannelCost(dbId) {
...
async function _renderChannelCostModal({ title, cost, saveBtn, onSave }) {
  openModal(`
  ...
  // Populate source dropdown from backend with pre-selected value
  const sourceSelect = document.getElementById('ccSource');
  const currentLang = state.currentLang || 'en';
  await populateSourceDropdown(sourceSelect, currentLang, cost.source);
}
