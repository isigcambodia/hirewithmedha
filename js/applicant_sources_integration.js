...
// Add this to your js/utils.js or create js/sources.js
// ============================================
...
/*
// Example 1: Populate dropdown on page load
document.addEventListener('DOMContentLoaded', async () => {
  const sourceSelect = document.getElementById('applicant-source');
  const currentLang = getCurrentLanguage(); // Your existing function
  await populateSourceDropdown(sourceSelect, currentLang);
});

// Example 2: Populate dropdown with pre-selected value
const sourceSelect = document.getElementById('applicant-source');
await populateSourceDropdown(sourceSelect, 'en', 'linkedin'); // Pre-select LinkedIn

// Example 3: Get display name for a source
const displayName = await getSourceDisplayName('employee_referral', 'km');
console.log(displayName); // "ការណែនាំពីបុគ្គលិក"

// Example 4: Load sources for custom rendering
const sources = await loadApplicantSources();
sources.forEach(source => {
  console.log(`${source.code}: ${source.name_en} / ${source.name_km}`);
});
*/
