...
## Summary
Replace hardcoded Function, Department, Section, and Role dropdowns with backend-driven data from four new Supabase lookup tables. The Role dropdown should display **only the job title name** (e.g., "Engineer, Electrical & Automation") — not the composite format with function and grade suffix.
