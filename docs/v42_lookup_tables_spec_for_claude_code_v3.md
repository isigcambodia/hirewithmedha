...
## Summary
Replace hardcoded Function, Department, Section, Role, and Grade dropdowns with backend-driven data from Supabase lookup tables. The Role dropdown should display **only the job title name** (e.g., "Engineer, Electrical & Automation") — not the composite format with function and grade suffix. Grade is **auto-filled and read-only** — when a role is selected, the grade is looked up from `company_job_grade` and displayed as a non-editable field. Requesters do not choose the grade.
...
| `company_job_grade` | 186 | — (text match on `job_title`) | Maps each job title to its company grade |

**`company_job_grade` schema**: `id` (uuid PK), `tenant_id`, `job_title` (text, matches `lookup_job_titles.name`), `grade` (text, e.g. "4"), `created_at`. Unique on `(tenant_id, job_title)`.
...
### Load company job grade mapping (load once on form mount)
```js
const { data } = await sb
  .from('company_job_grade')
  .select('job_title, grade')
  .eq('tenant_id', tenantId);
// Store as a map for quick lookup: { "Engineer, Electrical & Automation": "4", ... }
const gradeMap = Object.fromEntries(data.map(r => [r.job_title, r.grade]));
```
...
### Grade auto-fill (read-only)
- When a role is selected, look up the grade from `company_job_grade` using the selected job title name: `gradeMap[selectedJobTitleName]`
- Display the grade as a **read-only text field or disabled dropdown** — the requester cannot change it
- Label: "Company Job Grade" (EN) / "ថ្នាក់ការងារក្រុមហ៊ុន" (KH)
- If no mapping is found (shouldn't happen, but defensive), show empty/blank
- When role selection is cleared, clear the grade field too
...
```js
{
  ...
  grade: gradeMap[selectedJobTitleName]  // "4" (text, auto-filled from company_job_grade, not user-selected)
}
```
